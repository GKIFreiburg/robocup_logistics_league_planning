...
bool SyncActionInterface::lookupNumericalValue(rosplan_knowledge_msgs::KnowledgeItem& num)
{
...
bool SyncActionInterface::updateNumericalValue(rosplan_knowledge_msgs::KnowledgeItem& num)
{
...
void SyncActionInterface::runActionInterface()
{
	...
	for (; nit != predicateNames.end(); nit++)
	{
		...
		{
			predicates.insert(
					std::pair<std::string, rosplan_knowledge_msgs::DomainFormula>(*nit, predSrv.response.predicate));
		}
		else
		{
...
/* run action interface */
void SyncActionInterface::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{

	// check action name
	if (0 != msg->name.compare(params.name))
		return;
	...
	if (action_success)
	{
		...
		for (int i = 0; i < op.at_start_del_effects.size(); i++)
		{
			...
			if (!update_knowledge_client.call(updatePredSrv))
				ROS_INFO("KCL: (%s) failed to update PDDL model in knowledge base: %s", params.name.c_str(),
						op.at_start_del_effects[i].name.c_str());
		}
		...
		for (int i = 0; i < op.at_start_add_effects.size(); i++)
		{
			...
			if (!update_knowledge_client.call(updatePredSrv))
				ROS_INFO("KCL: (%s) failed to update PDDL model in knowledge base: %s", params.name.c_str(),
						op.at_start_add_effects[i].name.c_str());
		}
		...
		for (int i = 0; i < op.at_end_del_effects.size(); i++)
		{
			...
			if (!update_knowledge_client.call(updatePredSrv))
				ROS_INFO("KCL: (%s) failed to update PDDL model in knowledge base: %s", params.name.c_str(),
						op.at_end_del_effects[i].name.c_str());
		}
		...
		for (int i = 0; i < op.at_end_add_effects.size(); i++)
		{
			...
			if (!update_knowledge_client.call(updatePredSrv))
				ROS_INFO("KCL: (%s) failed to update PDDL model in knowledge base: %s", params.name.c_str(),
						op.at_end_add_effects[i].name.c_str());
		}
