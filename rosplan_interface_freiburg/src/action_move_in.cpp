...
typedef enum
{
	ACTION_ENABLED, ACTION_ACHIEVED, ACTION_FAILED
} ActionStatus;
...
public:
	ROSPlanInterfaceBehaviorEngine(ros::NodeHandle &n) :
			n(n), skiller_client_(n, "skiller", /* spin thread */false)
	{
		sub_action_dispatch_ = n.subscribe("/kcl_rosplan/action_dispatch", 10,
				&ROSPlanInterfaceBehaviorEngine::action_dispatch_cb, this);
		pub_action_feedback_ = n.advertise<rosplan_dispatch_msgs::ActionFeedback>("/kcl_rosplan/action_feedback", 10, true);

		svc_update_knowledge_ = n.serviceClient<rosplan_knowledge_msgs::KnowledgeUpdateService>(
		...
		if (!privn.getParam("team_color", team_color_))
		{
			n.getParam("team_color", team_color_);
		}
		...
		if (!cfg_igneffect_preds_.empty())
		{
			...
			std::string pred_str;
			std::for_each(cfg_igneffect_preds_.begin(), cfg_igneffect_preds_.end(), [&pred_str](const auto &p)
			{	pred_str += " " + p;});
		...
		if (!cfg_succeed_actions_.empty())
		{
			std::string act_str;
			bool first = true;
			std::for_each(cfg_succeed_actions_.begin(), cfg_succeed_actions_.end(), [&act_str, &first](const auto &a)
			{
	...
	void get_action_spec(const std::string &name)
	{
		ros::ServiceClient opdetail_client = n.serviceClient<rosplan_knowledge_msgs::GetDomainOperatorDetailsService>(
		...
		if (opdetail_client.call(srv))
		{
					...
					if (p.key == cfg_robot_var_name_)
					{
						...
						else
						{
							ROS_WARN("[RPI-BE] Action mapping '%s' has identifier variable '%s' of wrong type "
									"(got: %s, expected: %s), ignoring action", name.c_str(), p.key.c_str(), p.value.c_str(),
									cfg_robot_var_type_.c_str());
						}
					}
				}
			}
			...
			{
				specs_[name] =
				{	srv.response.op.formula, srv.response.op, reqp};
			}
			...
			const std::vector<rosplan_knowledge_msgs::DomainFormula> &df)
	{
		std::for_each(df.begin(), df.end(),
				[&predicate_names](auto &f)
				{	predicate_names.insert(f.name);});
	}
	...
	void
	action_dispatch_cb(const rosplan_dispatch_msgs::ActionDispatch::Ptr& msg)
	{
		...
		if(name.compare(action) == 0)
		{
			...
			std::string skill_string = map_skill("enter-field", msg->parameters);

			std::string param_str;
			std::for_each(msg->parameters.begin(), msg->parameters.end(),
					[&param_str](const auto &kv)
					{	param_str += " " + kv.key + "=" + kv.value;});
